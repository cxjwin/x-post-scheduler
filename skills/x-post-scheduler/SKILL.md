...
- 标点、排版配额与结构套路走统一的**反 AI 味格式规范**：`.claude/skills/x-post-scheduler/references/writing-format.md`——短推重点看标点铁律与禁用词；长文/线程发布前按文末 L1 清单机械扫一遍
...
# --gist-links 默认 body：每张代码图下挂「复制 代码块 N」标题锚点深链 + 文末总链接，发布即闭环；
# comment 为保守选项：正文零外链、gist 链接走发布后手动首评（脚本结尾打印首评文本）。
...
- **封面规格（账号主人在 X 编辑器实测确认，2026-08）**：比例 **5:2**（如 1536×614 / 1600×640）——注意两条海报通道的产出都不是这个比例（AI 海报 3:2、模板海报 16:9），直接拿海报当封面必被裁。封面走 Codex 的「imagegen 生成无字底图 → 本地裁切到 5:2 → Pillow 叠字」流程，最终文件必须精确 5:2
- **封面构图规范（与短推海报不同，账号主人定）**：必须带**文章标题**（主视觉，白色粗体，可两行）；**不放仓库链接行/日期行**（那是短推海报的格式）；要点可选、横排一行小字用「 · 」分隔；左下角 handle 署名；上传后在 X 编辑器核对裁切预览
...
- **gist 链接默认挂正文（`--gist-links body`，2026-08-12 账号主人拍板）**：每张代码图下挂「复制 代码块 N」深链（`#代码块-N` percent-encoded 标题锚点直达对应小节）+ 文末总链接。依据：X 官方已确认**取消了带链接帖子的硬性降权**（2026-07-29 Musk 原话「We haven't for over a year」，产品负责人 Nikita Bier 多次确认，配套 2025 年中的应用内 webview）；这类高相关工具性链接的间接影响有限，且 body 模式**发布即闭环**——不依赖发布后手动首评（X 政策禁止 API 发回复，comment 模式永远差人工一下）。已知代价：**Article 是吃停留时长（dwell time）的格式**，中途跳出会稀释互动信号、纯原生长文在推荐上仍有优势——在意这点时用 `--gist-links comment`（正文零外链，文末只留纯文本提示，链接走发布后手动首评）。**待办**：两种模式各发几篇同类文对比实际曝光，用数据回头校准默认值
- **首评补 gist 链接（comment 模式下必做）**：X Article 发布后是一条普通 status，可用短推的 `settings.reply_to_url` 给它建**回复草稿**当首评——但 **X 政策禁止经 API 发布/排期回复（实测 403 FORBIDDEN，2026-08-12），只允许创建草稿**，最后一下必须人在 Typefully 里点。推荐流程：`--publish-at now --auto-comment` 让脚本在确认发布后自动建好首评回复草稿并打印一键发布入口；排期发布（脚本不在场）或草稿创建失败时，脚本打印首评文本「本文代码可复制版 → <gist url>」，按 `pending-replies.md` 格式记下，发出后手动补（或用 `--text-file` + `--reply-to-url` 手动建回复草稿）。**comment 模式下这条首评是读者唯一的复制入口，漏发 = 复制通道断掉**，发布后报告里必须显式提醒；body 模式（默认）下它只是补充曝光，可选
