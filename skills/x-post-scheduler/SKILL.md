...
- **gist 链接默认不进正文（`--gist-links comment`）**——依据于 2026-08 更新：X 官方已确认**取消了带链接帖子的硬性降权**（2026-07-29 Musk 原话「We haven't for over a year」，产品负责人 Nikita Bier 多次确认，配套 2025 年中的应用内 webview）。但这不等于外链无成本：**Article 是吃停留时长（dwell time）的格式**，中途跳出会稀释互动信号，纯原生长文在推荐上仍有优势——所以默认仍走 comment：正文只在文末留一句**纯文本**提示「全部代码块的可复制版本见评论区」（无任何链接），gist 链接由发布后手动补的首评承载。代码密集、读者复制需求强的教程文可用 `--gist-links body`：每张代码图下挂「复制 代码块 N」深链（`#代码块-N` percent-encoded 标题锚点直达对应小节）+ 文末总链接——这类高相关工具性链接的间接影响有限，且 body 模式发布即闭环（不依赖手动首评）。**待办**：两种模式各发几篇同类文对比实际曝光，用数据定长期默认值
