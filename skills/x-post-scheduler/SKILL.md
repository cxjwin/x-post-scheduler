...
- **封面规格**（X 官方未公布确切像素，以下为 2026 年第三方指南共识 + 本仓库产出对照）：推荐 **1600×900（16:9）**；**时间线卡片比文章页裁得更紧**，关键文字务必居中、四边留牺牲空间。注意规格错配：AI 海报通道产出 1536×1024（**3:2**，gpt-image-2 尺寸限制），作封面被裁 16:9 时上下各损失约 80px——顶部小字行和底部署名放进安全区，或改用模板海报（`render.js` 原生 1200×675 即 16:9）；上传时在 X 编辑器里核对裁切预览
